...
ClkLog支持多个应用的数据采集，各个应用采集的数据从数据的查询、统计、分析及展示层面进行的数据隔离。ClkLog目前版本暂未接入用户系统，后续接入用户系统及应用权限后，每个应用可以由不同的用户进行管理，每个用户可以独立管理查看自己权限范围内的应用的埋点数据统计。
...
   登录ClkLog后台【元数据管理】，将整理好的数据采集模板导入对应应用的元数据（事件、事件属性、用户属性）表中。

   **注意：**

- Ø  在ClkLog元数据表中，元数据是跟应用有关的，每个应用有自己独立的元数据定义，在导入过程中请注意确认元数据导入的应用。
